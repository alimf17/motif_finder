...
// - This needs to find all the cross validation runs
// - This then needs to find the "best" set in each of the three ways I define best for each cross validation
// - This needs to take the file name of the omitted set and use it to generate the test set name 
// - This needs to calculate the eth statistic on the test set 
// - This needs to output a file with a list of locations to omit when calculating hits from FIMO vs peaks for the R script to reference 
...
/*
    ...
    for chain in 0..num_chains {
        ...
        let skip_some: bool = if min_chain == 0 && n_fold > 1 { true } else {false};

        let mut iter_files = chain_files.iter();


        if skip_some { 
            for _ in 0..(n_fold.min(10)){
            _ = iter_files.next(); 
            }
        }
    ...
    let plot_eth_file = fs::File::create(format!("{}/{}_approx_eth.svg", out_dir.clone(), base_file).as_str()).unwrap();
    for (i, trace) in set_trace_collections.iter().enumerate() {
        let letter = UPPER_LETTERS[i];
        plot_eth.line(format!("Chain {}", letter), trace.approx_ln_eth_trace(&data_ref).into_iter().enumerate().map(|(a, b)| (a as f64, b)));//.xmarker(0).ymarker(0);
    }; 
