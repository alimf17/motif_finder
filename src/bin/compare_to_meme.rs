...
fn main() {
    ...
    let motif_set_sup_set = args[3].clone();
    ...
    let stripped = StrippedMotifSet::from(&mot_set);
    let assigned = stripped_sup.pair_to_other_set_binding(&stripped, &data);

    println!("stripped sup {:?}", stripped_sup);
    println!("Assign complete {}", assigned.len());
    for (i, check) in assigned.into_iter().enumerate() {

        if let Some((rmse, sup_mot)) = check {
            println!("{i} {} {} {} {} {}", mot_set.get_nth_motif(i).peak_height(), sup_mot.peak_height(), mot_set.get_nth_motif(i).best_motif_string(), sup_mot.best_motif_string(), rmse);
            println!("{i} {} {} {} {} {}", stripped_sup.get_nth_motif(i).peak_height(), sup_mot.peak_height(), stripped_sup.get_nth_motif(i).best_motif_string(), sup_mot.best_motif_string(), rmse);
        };
